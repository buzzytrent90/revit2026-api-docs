...
# RebarBendingDetailResetTagRelativePosition Method  
...
```csharp
public static void ResetTagRelativePosition(
	Element bendingDetail
)
```
```vb
Public Shared Sub ResetTagRelativePosition ( 
	bendingDetail As Element
)
```
```cpp
public:
static void ResetTagRelativePosition(
	Element^ bendingDetail
)
```
```fsharp
static member ResetTagRelativePosition : 
