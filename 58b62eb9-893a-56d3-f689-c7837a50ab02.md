

Revit 2026 API

# ViewScheduleGetScheduleInstances Method  
  
---  
  
Gets the instances ids of schedule or schedule segment. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public IList<ElementId> GetScheduleInstances(
	int segmentIndex
)
```
```vb
Public Function GetScheduleInstances ( 
	segmentIndex As Integer
) As IList(Of ElementId)
```
```cpp
public:
IList<ElementId^>^ GetScheduleInstances(
	int segmentIndex
)
```
```fsharp
member GetScheduleInstances : 
        segmentIndex : int -> IList<ElementId> 
```


#### Parameters

segmentIndex Int32
     Index of the segment. 

#### Return Value

IList[ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md) The array of schedule sheet instance element ids of schedule or schedule segment. 

| Exception | Condition |
| --- | --- |
| [ArgumentException](2e6e4206-97a8-dd4b-df5d-4269f4bb6088.md) | The segment index should start from -1 and be less than the total segment count. |
  
The segment index value could be -1, it means to get schedule instances for the entire schedule. 

When a (primary) schedule is set to filter by sheet and placed on a sheet, it will create a new schedule with elements visible in the Viewport(s) on that sheet. The instance created belongs to the newly created schedule. Calls to GetScheduleInstances() will return instances of the newly created schedule but no instances of the primary schedule. 

#### Reference

[ViewSchedule Class](0dae24ba-5dcb-9a34-cccc-0cf8cc52bcd3.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)