

Revit 2026 API

# SpaceActualLightingLoad Property  
  
---  
  
Get the Actual Lighting Load of the Space.

**Namespace:** [Autodesk.Revit.DB.Mechanical](0eafd899-5912-56fd-94b1-d286156e26fc.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public double ActualLightingLoad { get; }
```
```vb
Public ReadOnly Property ActualLightingLoad As Double
	Get
```
```cpp
public:
property double ActualLightingLoad {
	double get ();
}
```
```fsharp
member ActualLightingLoad : float with get
```


#### Property Value

Double

This property is used to get the Actual Lighting Load of the Space.

#### Reference

[Space Class](b2c8970e-e554-8d73-06db-b65712c8a2e5.md)

[Autodesk.Revit.DB.Mechanical Namespace](0eafd899-5912-56fd-94b1-d286156e26fc.md)