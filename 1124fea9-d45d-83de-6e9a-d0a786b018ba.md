

Revit 2026 API

# RebarSetCrankStraightLengthOverride Method  
  
---  
  
Sets the crank straight length at the indicated bar end as an override value. 

**Namespace:** [Autodesk.Revit.DB.Structure](d586b341-f687-9d90-e96d-255806b7d4fc.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public void SetCrankStraightLengthOverride(
	int end,
	double crankStraightLength
)
```
```vb
Public Sub SetCrankStraightLengthOverride ( 
	end As Integer,
	crankStraightLength As Double
)
```
```cpp
public:
void SetCrankStraightLengthOverride(
	int end, 
	double crankStraightLength
)
```
```fsharp
member SetCrankStraightLengthOverride : 
        end : int * 
        crankStraightLength : float -> unit 
```


#### Parameters

end Int32
     The bar end. 
crankStraightLength Double
     The crank straigth length. 

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | end must be 0 or 1. |
| [InvalidOperationException](9e715f03-3884-e539-4dd6-8d7545733adc.md) | The bar does not have a valid Rebar Crank Type at the end. -or- The ability to override crank lengths is not enabled for this rebar instance. Use enableCrankLengthOverride(true) to enable it. |
  
When the crank straight length is set, the crank ratio is keeping the same value. The offset length and angled length are adjusted. 

#### Reference

[Rebar Class](70fd7426-f4a4-591c-8c06-3c18dda45e7d.md)

[Autodesk.Revit.DB.Structure Namespace](d586b341-f687-9d90-e96d-255806b7d4fc.md)