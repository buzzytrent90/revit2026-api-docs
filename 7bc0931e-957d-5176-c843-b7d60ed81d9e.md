

Revit 2026 API

# MEPNetworkSegmentDataGetUpstreamSegments Method  
  
---  
  
Gets all upstream network segments. 

**Namespace:** [Autodesk.Revit.DB.Analysis](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public IList<MEPNetworkSegmentId> GetUpstreamSegments()
```
```vb
Public Function GetUpstreamSegments As IList(Of MEPNetworkSegmentId)
```
```cpp
public:
IList<MEPNetworkSegmentId^>^ GetUpstreamSegments()
```
```fsharp
member GetUpstreamSegments : unit -> IList<MEPNetworkSegmentId> 
```


#### Return Value

IList[MEPNetworkSegmentId](cb14904b-7147-4742-09c9-98da77011030.md) The list of segments that flow into this segment. 

#### Reference

[MEPNetworkSegmentData Class](26acc393-12fd-3d27-ee25-f21c88341190.md)

[Autodesk.Revit.DB.Analysis Namespace](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)