

Revit 2026 API

# EnergyDataSettingsCheckAnalysisType Method  
  
---  
  
Checks that the analysis type falls within an appropriate range. 

**Namespace:** [Autodesk.Revit.DB.Analysis](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static bool CheckAnalysisType(
	AnalysisMode analysisType
)
```
```vb
Public Shared Function CheckAnalysisType ( 
	analysisType As AnalysisMode
) As Boolean
```
```cpp
public:
static bool CheckAnalysisType(
	AnalysisMode analysisType
)
```
```fsharp
static member CheckAnalysisType : 
        analysisType : AnalysisMode -> bool 
```


#### Parameters

analysisType [AnalysisMode](a4d5128f-91e8-06f4-c5c5-72ba0f705a48.md)
     The analysis type to be checked. 

#### Return Value

Boolean True if the analysis type falls within an appropriate range, false otherwise. 

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | A value passed for an enumeration argument is not a member of that enumeration |
  
#### Reference

[EnergyDataSettings Class](63628109-daa0-e5b2-3dfd-153179e54816.md)

[Autodesk.Revit.DB.Analysis Namespace](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)