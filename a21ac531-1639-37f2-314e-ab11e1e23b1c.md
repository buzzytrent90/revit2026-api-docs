...
# AdvancedLossFactorVoltageLossFactor Property  
...
```csharp
public double VoltageLossFactor { get; set; }
```
```vb
Public Property VoltageLossFactor As Double
	Get
	Set
```
```cpp
public:
property double VoltageLossFactor {
	double get ();
	void set (double value);
}
```
```fsharp
member VoltageLossFactor : float with get, set
```
