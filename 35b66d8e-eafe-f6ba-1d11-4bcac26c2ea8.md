...
# ElectricalSystemApparentLoadPhaseA Property  
...
```csharp
public double ApparentLoadPhaseA { get; }
```
```vb
Public ReadOnly Property ApparentLoadPhaseA As Double
	Get
```
```cpp
public:
property double ApparentLoadPhaseA {
	double get ();
}
```
```fsharp
member ApparentLoadPhaseA : float with get
```
