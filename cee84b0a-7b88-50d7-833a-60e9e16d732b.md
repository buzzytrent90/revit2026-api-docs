...
# RebarBarTypeSetAutoCalculatedLapLength Method  
  
---  
  
Sets if the lap length is auto calculated or not for the specified Rebar Splice Type. 
...
```csharp
public void SetAutoCalculatedLapLength(
	ElementId rebarSpliceTypeId,
	bool autoCalculated
)
```
```vb
Public Sub SetAutoCalculatedLapLength ( 
...
```cpp
public:
void SetAutoCalculatedLapLength(
	ElementId^ rebarSpliceTypeId, 
	bool autoCalculated
)
```
```fsharp
member SetAutoCalculatedLapLength : 
...
autoCalculated Boolean
     True, if the lap length is auto calculated, false otherwise. 
