

Revit 2026 API

# TileTileRandomShift Property  
  
---  
  
The property labeled "Random" from the "Tile" schema. 

**Namespace:** [Autodesk.Revit.DB.Visual](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static string TileRandomShift { get; }
```
```vb
Public Shared ReadOnly Property TileRandomShift As String
	Get
```
```cpp
public:
static property String^ TileRandomShift {
	String^ get ();
}
```
```fsharp
static member TileRandomShift : string with get
```


#### Property Value

String

This property is of type "AssetPropertyDouble" within the range of "0, 100". 

#### Reference

[Tile Class](2871b849-9a05-b097-d27d-b998e8254311.md)

[Autodesk.Revit.DB.Visual Namespace](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)