

Revit 2026 API

# UnitUtilsGetAllDisciplines Method  
  
---  
  
Gets the identifiers of all available disciplines. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static IList<ForgeTypeId> GetAllDisciplines()
```
```vb
Public Shared Function GetAllDisciplines As IList(Of ForgeTypeId)
```
```cpp
public:
static IList<ForgeTypeId^>^ GetAllDisciplines()
```
```fsharp
static member GetAllDisciplines : unit -> IList<ForgeTypeId> 
```


#### Return Value

IList[ForgeTypeId](d9fcf276-9566-de83-2b0b-d89b65ccc8af.md) The discipline identifiers. 

#### Reference

[UnitUtils Class](128dd879-fea8-5d7b-1eb2-d64f87753990.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)