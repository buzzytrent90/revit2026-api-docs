...
# ElectricalSystemApparentLoadPhaseB Property  
...
```csharp
public double ApparentLoadPhaseB { get; }
```
```vb
Public ReadOnly Property ApparentLoadPhaseB As Double
	Get
```
```cpp
public:
property double ApparentLoadPhaseB {
	double get ();
}
```
```fsharp
member ApparentLoadPhaseB : float with get
```
