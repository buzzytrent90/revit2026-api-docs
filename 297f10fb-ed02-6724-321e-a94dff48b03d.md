

Revit 2026 API

# GenericGenericReflectivityAt0deg Property  
  
---  
  
The property labeled "Reflectivity/Direct" from the "Generic" schema. 

**Namespace:** [Autodesk.Revit.DB.Visual](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static string GenericReflectivityAt0deg { get; }
```
```vb
Public Shared ReadOnly Property GenericReflectivityAt0deg As String
	Get
```
```cpp
public:
static property String^ GenericReflectivityAt0deg {
	String^ get ();
}
```
```fsharp
static member GenericReflectivityAt0deg : string with get
```


#### Property Value

String

This property is of type "AssetPropertyDouble". This property allows a connected asset. 

#### Reference

[Generic Class](dd16eb59-16ec-f121-289b-a69d26d7c789.md)

[Autodesk.Revit.DB.Visual Namespace](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)