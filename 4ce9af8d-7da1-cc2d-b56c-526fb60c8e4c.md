...
# ElectricalSystemTrueLoadPhaseC Property  
...
```csharp
public double TrueLoadPhaseC { get; }
```
```vb
Public ReadOnly Property TrueLoadPhaseC As Double
	Get
```
```cpp
public:
property double TrueLoadPhaseC {
	double get ();
}
```
```fsharp
member TrueLoadPhaseC : float with get
```
