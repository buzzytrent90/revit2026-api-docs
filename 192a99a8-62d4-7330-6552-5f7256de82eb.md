

Revit 2026 API

# WorksharingDisplaySettingsGetGraphicOverrides(ModelUpdatesStatus) Method  
  
---  
  
Returns the graphic overrides assigned to a particular model update status. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public WorksharingDisplayGraphicSettings GetGraphicOverrides(
	ModelUpdatesStatus statusInCentral
)
```
```vb
Public Function GetGraphicOverrides ( 
	statusInCentral As ModelUpdatesStatus
) As WorksharingDisplayGraphicSettings
```
```cpp
public:
WorksharingDisplayGraphicSettings^ GetGraphicOverrides(
	ModelUpdatesStatus statusInCentral
)
```
```fsharp
member GetGraphicOverrides : 
        statusInCentral : ModelUpdatesStatus -> WorksharingDisplayGraphicSettings 
```


#### Parameters

statusInCentral [ModelUpdatesStatus](2fc35e00-cdf2-8368-e6a6-032725492cbe.md)
     The model update status of interest. 

#### Return Value

[WorksharingDisplayGraphicSettings](994d2fb5-11cc-6756-155b-d496eedbe800.md) Returns the graphic overrides assigned to the model update status. 

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | A value passed for an enumeration argument is not a member of that enumeration |
  
#### Reference

[WorksharingDisplaySettings Class](ec25e291-6582-7e8c-f273-efc0c391bcc4.md)

[GetGraphicOverrides Overload](ce69efb3-a7da-166e-5f00-c306236de4b5.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)