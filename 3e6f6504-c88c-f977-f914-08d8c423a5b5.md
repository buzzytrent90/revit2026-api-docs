

Revit 2026 API

# FabricationPartTopOfPartElevation Property  
  
---  
  
The associated elevation to the top of fabrication part off of the current level. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public double TopOfPartElevation { get; }
```
```vb
Public ReadOnly Property TopOfPartElevation As Double
	Get
```
```cpp
public:
property double TopOfPartElevation {
	double get ();
}
```
```fsharp
member TopOfPartElevation : float with get
```


#### Property Value

Double

#### Reference

[FabricationPart Class](c9b86162-c105-696a-a919-49a7a7938cc4.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)