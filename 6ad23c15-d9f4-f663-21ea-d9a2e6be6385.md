...
# ElectricalSystemApparentLoadPhaseC Property  
...
```csharp
public double ApparentLoadPhaseC { get; }
```
```vb
Public ReadOnly Property ApparentLoadPhaseC As Double
	Get
```
```cpp
public:
property double ApparentLoadPhaseC {
	double get ();
}
```
```fsharp
member ApparentLoadPhaseC : float with get
```
