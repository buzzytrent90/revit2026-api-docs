

Revit 2026 API

# CategorySetLinePatternId Method  
  
---  
  
Sets the line pattern id associated with this category for the given graphics style type.

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public void SetLinePatternId(
	ElementId linePatternId,
	GraphicsStyleType graphicsStyleType
)
```
```vb
Public Sub SetLinePatternId ( 
	linePatternId As ElementId,
	graphicsStyleType As GraphicsStyleType
)
```
```cpp
public:
void SetLinePatternId(
	ElementId^ linePatternId, 
	GraphicsStyleType graphicsStyleType
)
```
```fsharp
member SetLinePatternId : 
        linePatternId : ElementId * 
        graphicsStyleType : GraphicsStyleType -> unit 
```


#### Parameters

linePatternId [ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md)
    The line pattern id for the graphics style.
graphicsStyleType [GraphicsStyleType](60a5cce6-2cdc-dd63-e737-f584582ceeca.md)
    The type of graphics style.

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | Thrown when the input argument "linePatternId" is an illegal id. |
| [InvalidOperationException](9e715f03-3884-e539-4dd6-8d7545733adc.md) | Thrown when this category does not have stored line pattern id for this graphics style type. |
  
  * The line pattern id will be one of the following:
  * A negative value (representing a built-in line pattern); this value can only be obtained via GetLinePatternId
  * The id of a LinePatternElement



#### Reference

[Category Class](d390ecf6-e5db-d7c1-d7f2-766c0686e975.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)