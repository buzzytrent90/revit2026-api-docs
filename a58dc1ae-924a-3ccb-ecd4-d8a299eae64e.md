

Revit 2026 API

# BuiltInFailuresEnergyAnalysisFailuresProblemWithLightingLoadWarn Property  
  
---  
  
[Room]'s lighting load is too high. This may be caused by an incorrect density value and must be corrected before this space can be included in the load calculations. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static FailureDefinitionId ProblemWithLightingLoadWarn { get; }
```
```vb
Public Shared ReadOnly Property ProblemWithLightingLoadWarn As FailureDefinitionId
	Get
```
```cpp
public:
static property FailureDefinitionId^ ProblemWithLightingLoadWarn {
	FailureDefinitionId^ get ();
}
```
```fsharp
static member ProblemWithLightingLoadWarn : FailureDefinitionId with get
```


#### Property Value

[FailureDefinitionId](b6ada360-a6fe-ebb6-b095-d74b37ade9bf.md)

#### Reference

[BuiltInFailuresEnergyAnalysisFailures Class](8b9bfa39-1c9b-5cb0-14f1-0f49e2f8828a.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)