

Revit 2026 API

# ExporterIFCFindSpaceBoundingElementHandle Method  
  
---  
  
Looks up the handle associated to the element and level id from the ExporterIFC's internal cache. 

**Namespace:** [Autodesk.Revit.DB.IFC](b823fafb-1ba1-896b-4097-142c2817ce74.md)**Assembly:** RevitAPIIFC (in RevitAPIIFC.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public IFCAnyHandle FindSpaceBoundingElementHandle(
	ElementId id,
	ElementId levelId
)
```
```vb
Public Function FindSpaceBoundingElementHandle ( 
	id As ElementId,
	levelId As ElementId
) As IFCAnyHandle
```
```cpp
public:
IFCAnyHandle^ FindSpaceBoundingElementHandle(
	ElementId^ id, 
	ElementId^ levelId
)
```
```fsharp
member FindSpaceBoundingElementHandle : 
        id : ElementId * 
        levelId : ElementId -> IFCAnyHandle 
```


#### Parameters

id [ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md)
     The Revit element id to look for. 
levelId [ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md)
     The element level id. 

#### Return Value

[IFCAnyHandle](8b893943-70fa-94bf-90be-1523d516ecb3.md) The handle associated to the element and level id. If the id is not found in the cache, an empty handle is returned (HasValue == false). 

| Exception | Condition |
| --- | --- |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
  
#### Reference

[ExporterIFC Class](c8697b81-e080-9202-14d3-ec883f951521.md)

[Autodesk.Revit.DB.IFC Namespace](b823fafb-1ba1-896b-4097-142c2817ce74.md)