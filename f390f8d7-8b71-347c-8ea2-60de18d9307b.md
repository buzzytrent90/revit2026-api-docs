

Revit 2026 API

# CableSizeNumberOfOtherConductors Property  
  
---  
  
The number of Other Conductors. 

**Namespace:** [Autodesk.Revit.DB.Electrical](212a1314-7843-2c6c-3322-363127e4059f.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public int NumberOfOtherConductors { get; set; }
```
```vb
Public Property NumberOfOtherConductors As Integer
	Get
	Set
```
```cpp
public:
property int NumberOfOtherConductors {
	int get ();
	void set (int value);
}
```
```fsharp
member NumberOfOtherConductors : int with get, set
```


#### Property Value

Int32

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | When setting this property: The given value for numberOfOtherConductors is negative. |
  
#### Reference

[CableSize Class](a6a35ad4-346a-0711-d38d-8b360c19c9e2.md)

[Autodesk.Revit.DB.Electrical Namespace](212a1314-7843-2c6c-3322-363127e4059f.md)