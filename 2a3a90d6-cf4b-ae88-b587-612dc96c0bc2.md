

Revit 2026 API

# ExternalDefinitionCreationOptionsHideWhenNoValue Property  
  
---  
  
This property indicates whether this parameter should be hidden from the properties palette when it doesn't have a value. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public bool HideWhenNoValue { get; set; }
```
```vb
Public Property HideWhenNoValue As Boolean
	Get
	Set
```
```cpp
public:
property bool HideWhenNoValue {
	bool get ();
	void set (bool value);
}
```
```fsharp
member HideWhenNoValue : bool with get, set
```


#### Property Value

Boolean True if the parameter should be hidden when it has no value, False if the parameter should remain visible in the properties palette even when it doesn't have a value. The default is false 

#### Reference

[ExternalDefinitionCreationOptions Class](1cd9e425-23a3-04f8-c130-4d4a799abd13.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)