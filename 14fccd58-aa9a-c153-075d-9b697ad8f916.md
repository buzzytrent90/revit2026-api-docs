

Revit 2026 API

# BuiltInFailuresDividedPathFailuresDividedPathIntersectionPolycurveParamsStepFailed Property  
  
---  
  
Intersection failed. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static FailureDefinitionId DividedPathIntersectionPolycurveParamsStepFailed { get; }
```
```vb
Public Shared ReadOnly Property DividedPathIntersectionPolycurveParamsStepFailed As FailureDefinitionId
	Get
```
```cpp
public:
static property FailureDefinitionId^ DividedPathIntersectionPolycurveParamsStepFailed {
	FailureDefinitionId^ get ();
}
```
```fsharp
static member DividedPathIntersectionPolycurveParamsStepFailed : FailureDefinitionId with get
```


#### Property Value

[FailureDefinitionId](b6ada360-a6fe-ebb6-b095-d74b37ade9bf.md)

#### Reference

[BuiltInFailuresDividedPathFailures Class](96557f40-4ea3-021a-cde7-1f67aeef1a16.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)