

Revit 2026 API

# EnergyDataSettingsCheckRangeOfSillHeight Method  
  
---  
  
Checks that the sill height is greater than or equal to zero. 

**Namespace:** [Autodesk.Revit.DB.Analysis](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static bool CheckRangeOfSillHeight(
	double sillHeight
)
```
```vb
Public Shared Function CheckRangeOfSillHeight ( 
	sillHeight As Double
) As Boolean
```
```cpp
public:
static bool CheckRangeOfSillHeight(
	double sillHeight
)
```
```fsharp
static member CheckRangeOfSillHeight : 
        sillHeight : float -> bool 
```


#### Parameters

sillHeight Double
     The sill height to be checked. 

#### Return Value

Boolean True if the sill height falls is greater than or equal to zero, false otherwise. 

#### Reference

[EnergyDataSettings Class](63628109-daa0-e5b2-3dfd-153179e54816.md)

[Autodesk.Revit.DB.Analysis Namespace](958e2e12-587d-f188-5d7b-f13d7dbfdf48.md)