...
# ElectricalSystemApparentCurrentPhaseC Property  
...
```csharp
public double ApparentCurrentPhaseC { get; }
```
```vb
Public ReadOnly Property ApparentCurrentPhaseC As Double
	Get
```
```cpp
public:
property double ApparentCurrentPhaseC {
	double get ();
}
```
```fsharp
member ApparentCurrentPhaseC : float with get
```
