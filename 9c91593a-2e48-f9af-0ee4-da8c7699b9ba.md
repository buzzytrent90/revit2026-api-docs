...
# ViewScheduleGetDefaultNameForMaterialTakeoff Method  
...
```
```vb
Public Shared Function GetDefaultNameForMaterialTakeoff ( 
...
```cpp
public:
static String^ GetDefaultNameForMaterialTakeoff(
	Document^ document, 
	ElementId^ categoryId
)
```
```fsharp
static member GetDefaultNameForMaterialTakeoff : 
        document : Document * 
...
| Exception | Condition |
| --- | --- |
| [ArgumentException](2e6e4206-97a8-dd4b-df5d-4269f4bb6088.md) | document is not a project document. -or- categoryId is not a valid category for a material takeoff. |
