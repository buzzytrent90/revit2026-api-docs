

Revit 2026 API

# MetalMetalPerforationsShader Property  
  
---  
  
The property labeled "Image" from the "Metal" schema. 

**Namespace:** [Autodesk.Revit.DB.Visual](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static string MetalPerforationsShader { get; }
```
```vb
Public Shared ReadOnly Property MetalPerforationsShader As String
	Get
```
```cpp
public:
static property String^ MetalPerforationsShader {
	String^ get ();
}
```
```fsharp
static member MetalPerforationsShader : string with get
```


#### Property Value

String

This property is of type "AssetPropertyReference" and will only contain a reference to a connected image. 

#### Reference

[Metal Class](618a6255-d79c-e405-6804-994c56317dc4.md)

[Autodesk.Revit.DB.Visual Namespace](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)