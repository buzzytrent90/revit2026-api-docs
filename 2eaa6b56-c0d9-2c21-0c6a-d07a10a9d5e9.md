

Revit 2026 API

# View3DScalePerspectiveCropBox Method  
  
---  
  
Scale an existing crop box of the perspective view. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public void ScalePerspectiveCropBox(
	double multiplier
)
```
```vb
Public Sub ScalePerspectiveCropBox ( 
	multiplier As Double
)
```
```cpp
public:
void ScalePerspectiveCropBox(
	double multiplier
)
```
```fsharp
member ScalePerspectiveCropBox : 
        multiplier : float -> unit 
```


#### Parameters

multiplier Double
     Multiplier to change the view scale and the current crop box size on both X and Y. 

| Exception | Condition |
| --- | --- |
| [InvalidOperationException](9e715f03-3884-e539-4dd6-8d7545733adc.md) | Returns true if the view is not a view template. |
  
In addition to changing the view crop box, this operation also makes the change analogous to changing the scale of the orthographic view, so that both the size and scale of the view on a sheet changes according to the provided argument 'multiplier'. 

For getting or setting view crop box size use property [CropBox](d6246051-ecfb-7388-0429-6ed65de72638.md)

#### Reference

[View3D Class](d795a238-fc24-1875-e64f-a2bef56ae949.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)