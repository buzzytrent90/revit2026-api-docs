...
# SpaceDesignExhaustAirflow Property  
...
```csharp
public double DesignExhaustAirflow { get; set; }
```
```vb
Public Property DesignExhaustAirflow As Double
	Get
	Set
```
```cpp
public:
property double DesignExhaustAirflow {
	double get ();
	void set (double value);
}
```
```fsharp
member DesignExhaustAirflow : float with get, set
```
