

Revit 2026 API

# FabricationPartReposition Method  
  
---  
  
Repositions the fabrication straight part to another end of the run. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static void Reposition(
	Document document,
	ElementId partId
)
```
```vb
Public Shared Sub Reposition ( 
	document As Document,
	partId As ElementId
)
```
```cpp
public:
static void Reposition(
	Document^ document, 
	ElementId^ partId
)
```
```fsharp
static member Reposition : 
        document : Document * 
        partId : ElementId -> unit 
```


#### Parameters

document [Document](db03274b-a107-aa32-9034-f3e0df4bb1ec.md)
     The document. 
partId [ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md)
     Identifier of the fabrication part to reposition. 

| Exception | Condition |
| --- | --- |
| [ArgumentException](2e6e4206-97a8-dd4b-df5d-4269f4bb6088.md) | It is not a straight segment on the end in the run. -or- There are locked parts in the run. |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
| [InvalidOperationException](9e715f03-3884-e539-4dd6-8d7545733adc.md) | this operation failed. |
  
#### Reference

[FabricationPart Class](c9b86162-c105-696a-a919-49a7a7938cc4.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)