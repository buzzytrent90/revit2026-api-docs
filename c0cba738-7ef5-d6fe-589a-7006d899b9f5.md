...
# PipingSystemTypeTwoLineDropType Property  
...
```
```vb
Public Property TwoLineDropType As RiseDropSymbol
	Get
	Set
```
```cpp
public:
property RiseDropSymbol TwoLineDropType {
	RiseDropSymbol get ();
	void set (RiseDropSymbol value);
}
```
```fsharp
member TwoLineDropType : RiseDropSymbol with get, set
```
