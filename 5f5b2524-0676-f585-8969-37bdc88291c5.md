

Revit 2026 API

# RebarBendingDetailTypeCreateSchematic Method  
  
---  
  
Creates a new RebarBendingDetailType object for schematic representations. 

**Namespace:** [Autodesk.Revit.DB.Structure](d586b341-f687-9d90-e96d-255806b7d4fc.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static RebarBendingDetailType CreateSchematic(
	Document Document
)
```
```vb
Public Shared Function CreateSchematic ( 
	Document As Document
) As RebarBendingDetailType
```
```cpp
public:
static RebarBendingDetailType^ CreateSchematic(
	Document^ Document
)
```
```fsharp
static member CreateSchematic : 
        Document : Document -> RebarBendingDetailType 
```


#### Parameters

Document [Document](db03274b-a107-aa32-9034-f3e0df4bb1ec.md)
     Document to which the new element should be added. 

#### Return Value

[RebarBendingDetailType](9e6af0fc-1c7a-47e2-d6fd-460a36d6bc89.md) The new RebarBendingDetailType object. 

| Exception | Condition |
| --- | --- |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
  
#### Reference

[RebarBendingDetailType Class](9e6af0fc-1c7a-47e2-d6fd-460a36d6bc89.md)

[Autodesk.Revit.DB.Structure Namespace](d586b341-f687-9d90-e96d-255806b7d4fc.md)