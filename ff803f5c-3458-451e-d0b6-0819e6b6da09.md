

Revit 2026 API

# TileTilePerColumn Property  
  
---  
  
The property labeled "Every" from the "Tile" schema. 

**Namespace:** [Autodesk.Revit.DB.Visual](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static string TilePerColumn { get; }
```
```vb
Public Shared ReadOnly Property TilePerColumn As String
	Get
```
```cpp
public:
static property String^ TilePerColumn {
	String^ get ();
}
```
```fsharp
static member TilePerColumn : string with get
```


#### Property Value

String

This property is of type "AssetPropertyInteger" within the range of "0, 50". 

#### Reference

[Tile Class](2871b849-9a05-b097-d27d-b998e8254311.md)

[Autodesk.Revit.DB.Visual Namespace](f5a10581-6ac2-be19-0e32-f87d05bc8b83.md)