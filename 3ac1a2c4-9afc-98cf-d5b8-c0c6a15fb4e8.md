

Revit 2026 API

# SelectionPickObjects(ObjectType, ISelectionFilter) Method  
  
---  
  
Prompts the user to select multiple objects which pass a customer filter.

**Namespace:** [Autodesk.Revit.UI.Selection](11785869-cc9e-03fc-97db-767a59af10a1.md)**Assembly:** RevitAPIUI (in RevitAPIUI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public IList<Reference> PickObjects(
	ObjectType objectType,
	ISelectionFilter selectionFilter
)
```
```vb
Public Function PickObjects ( 
	objectType As ObjectType,
	selectionFilter As ISelectionFilter
) As IList(Of Reference)
```
```cpp
public:
IList<Reference^>^ PickObjects(
	ObjectType objectType, 
	ISelectionFilter^ selectionFilter
)
```
```fsharp
member PickObjects : 
        objectType : ObjectType * 
        selectionFilter : ISelectionFilter -> IList<Reference> 
```


#### Parameters

objectType [ObjectType](2d0cbbba-d4ab-84b7-b081-36c14769d82c.md)
    Specifies the type of object to be selected.
selectionFilter [ISelectionFilter](d552f44b-221c-0ecd-d001-41a5099b2f9f.md)
    The selection filter.

#### Return Value

IList[Reference](d28155ae-817b-1f31-9c3f-c9c6a28acc0d.md)

A collection of references selected by the user.

Note: if the user cancels the operation (for example, through ESC), the method will throw an OperationCanceledException instance. 

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | Thrown when the objectType is not a recognized value. |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | Thrown when the selectionFilter is . |
| [OperationCanceledException](aea34480-ceb5-b49f-129d-0799e7bb1c21.md) | Thrown when the Revit user cancelled this operation. Thrown when the Revit user tried to switch the active view, close the active document or Revit application when responding to this mode. |
| [ForbiddenForDynamicUpdateException](c5b911f6-1e8f-2cd4-6965-286f41221fe0.md) | Thrown if this method is called during dynamic update. |
  
The user will be shown "Finish" and "Cancel" buttons on the dialog bar to complete the selection operation. Uncheck the "Multiple" check-box to select single object and it will return the selected object directly.

Revit users will be permitted to manipulate the Revit view (zooming, panning, and rotating the view), but will not be permitted to click other items in the Revit user interface. Users are not permitted to switch the active view, close the active document or Revit application in the pick session, otherwise an exception will be thrown.

The selection will not be automatically added to the active selection buffer.

Note: this method must not be called during dynamic update, otherwise ForbiddenForDynamicUpdateException will be thrown.

#### Reference

[Selection Class](31b73d46-7d67-5dbb-4dad-80aa597c9afc.md)

[PickObjects Overload](e5a547a2-3cf5-7638-2daa-ea85b4d3de2d.md)

[Autodesk.Revit.UI.Selection Namespace](11785869-cc9e-03fc-97db-767a59af10a1.md)