

Revit 2026 API

# FloorGetVerticalProjectionPoint Method  
  
---  
  
Return a surface point on either the top or bottom face of a floor slab corresponding to the vertical projection of an arbitrary point in project space. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public XYZ GetVerticalProjectionPoint(
	XYZ modelLocation,
	FloorFace floorFace
)
```
```vb
Public Function GetVerticalProjectionPoint ( 
	modelLocation As XYZ,
	floorFace As FloorFace
) As XYZ
```
```cpp
public:
XYZ^ GetVerticalProjectionPoint(
	XYZ^ modelLocation, 
	FloorFace floorFace
)
```
```fsharp
member GetVerticalProjectionPoint : 
        modelLocation : XYZ * 
        floorFace : FloorFace -> XYZ 
```


#### Parameters

modelLocation [XYZ](c2fd995c-95c0-58fb-f5de-f3246cbc5600.md)
     A point in project coordinates that will be projected to the slab top or bottom face. 
floorFace [FloorFace](7a34e4cb-0a5d-5299-813b-edfbe364953b.md)
     A flag determining whether the top or bottom face of the floor should be used. 

#### Return Value

[XYZ](c2fd995c-95c0-58fb-f5de-f3246cbc5600.md) Slab surface point for the vertically projected model point. 

If the floor is shape edited, the surface location must be within the boundaries of a face on the slab. Otherwise the method will return . 

#### Reference

[Floor Class](96cc6685-003d-ff90-1c5b-c25a4830f0f7.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)