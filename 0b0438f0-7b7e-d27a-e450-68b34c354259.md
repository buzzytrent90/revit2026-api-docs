...
# SpaceDesignLightingLoad Property  
...
```csharp
public double DesignLightingLoad { get; set; }
```
```vb
Public Property DesignLightingLoad As Double
	Get
	Set
```
```cpp
public:
property double DesignLightingLoad {
	double get ();
	void set (double value);
}
```
```fsharp
member DesignLightingLoad : float with get, set
```
