

Revit 2026 API

# CompoundStructureSetNumberOfShellLayers Method  
  
---  
  
Sets the number of interior or exterior shell layers. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public void SetNumberOfShellLayers(
	ShellLayerType shellLayerType,
	int numLayers
)
```
```vb
Public Sub SetNumberOfShellLayers ( 
	shellLayerType As ShellLayerType,
	numLayers As Integer
)
```
```cpp
public:
void SetNumberOfShellLayers(
	ShellLayerType shellLayerType, 
	int numLayers
)
```
```fsharp
member SetNumberOfShellLayers : 
        shellLayerType : ShellLayerType * 
        numLayers : int -> unit 
```


#### Parameters

shellLayerType [ShellLayerType](75c640b9-9904-7882-43fc-a4dfc25ff53c.md)
     If ShellLayerType.Exterior set the number of exterior shell layers (or top shell layers for a roof, floor, or ceiling type). If ShellLayerType.Interior set the number of interior shell layers (or bottom shell layers for a roof, floor, or ceiling type). 
numLayers Int32
     The number of layers to be in the specified shell. 

| Exception | Condition |
| --- | --- |
| [ArgumentOutOfRangeException](60f148c9-ece0-a6bb-4e12-bb4a9c8c8a24.md) | Number of shell layers is negative. -or- A value passed for an enumeration argument is not a member of that enumeration |
| [ArgumentsInconsistentException](05972c68-fa6d-3a83-d720-ad84fbc4780f.md) | Too many shell layers: Number of shell layers of this type is too large. |
  
Changing this value can result in Core layers becoming Shell layers. This can also result in all Core layers becoming Shell layers and there being an empty core. 

#### Reference

[CompoundStructure Class](dc1a081e-8dab-565f-145d-a429098d353c.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)