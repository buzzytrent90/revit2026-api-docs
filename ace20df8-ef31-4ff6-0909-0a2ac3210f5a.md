...
# ElectricalSystemTrueCurrentPhaseA Property  
...
```csharp
public double TrueCurrentPhaseA { get; }
```
```vb
Public ReadOnly Property TrueCurrentPhaseA As Double
	Get
```
```cpp
public:
property double TrueCurrentPhaseA {
	double get ();
}
```
```fsharp
member TrueCurrentPhaseA : float with get
```
