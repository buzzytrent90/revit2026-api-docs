

Revit 2026 API

# ParameterFilterRuleFactoryCreateGreaterRule(ElementId, Double, Double) Method  
  
---  
  
Creates a filter rule that determines whether double-precision values from the document are greater than a certain value. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public static FilterRule CreateGreaterRule(
	ElementId parameter,
	double value,
	double epsilon
)
```
```vb
Public Shared Function CreateGreaterRule ( 
	parameter As ElementId,
	value As Double,
	epsilon As Double
) As FilterRule
```
```cpp
public:
static FilterRule^ CreateGreaterRule(
	ElementId^ parameter, 
	double value, 
	double epsilon
)
```
```fsharp
static member CreateGreaterRule : 
        parameter : ElementId * 
        value : float * 
        epsilon : float -> FilterRule 
```


#### Parameters

parameter [ElementId](44f3f7b1-3229-3404-93c9-dc5e70337dd6.md)
     A double-typed parameter used to get values from the document for a given element. 
value Double
     The user-supplied value against which values from the document will be compared. 
epsilon Double
     Defines the tolerance within which two values may be considered equal. 

#### Return Value

[FilterRule](a8f202ca-3c88-ecc4-fa93-549b26a412d7.md)

| Exception | Condition |
| --- | --- |
| [ArgumentException](2e6e4206-97a8-dd4b-df5d-4269f4bb6088.md) | The given value for value is not finite -or- The given value for value is not a number -or- The given value for epsilon is not finite -or- The given value for epsilon is not a number |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
  
Values greater than _value_ but within _epsilon_ are considered equal, not greater. 

#### Reference

[ParameterFilterRuleFactory Class](317755a4-24ba-9f36-7639-f6fb2aa5a1a7.md)

[CreateGreaterRule Overload](3885f1a1-d26c-a3c3-54e1-81f75c04148a.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)