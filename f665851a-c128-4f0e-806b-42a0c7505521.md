...
# AlignmentGetHCurveNormalAtStation Method  
...
```
```vb
Public Function GetHCurveNormalAtStation ( 
	station As Double
) As XYZ
```
```cpp
public:
XYZ^ GetHCurveNormalAtStation(
	double station
)
```
```fsharp
member GetHCurveNormalAtStation : 
        station : float -> XYZ 
```
...
[XYZ](c2fd995c-95c0-58fb-f5de-f3246cbc5600.md)2d (X and Y components of returned vector) horizontal normal at given station.
