...
# ElectricalSystemTrueLoadPhaseA Property  
...
```csharp
public double TrueLoadPhaseA { get; }
```
```vb
Public ReadOnly Property TrueLoadPhaseA As Double
	Get
```
```cpp
public:
property double TrueLoadPhaseA {
	double get ();
}
```
```fsharp
member TrueLoadPhaseA : float with get
```
