...
# ElectricalSystemTrueCurrentPhaseC Property  
...
```csharp
public double TrueCurrentPhaseC { get; }
```
```vb
Public ReadOnly Property TrueCurrentPhaseC As Double
	Get
```
```cpp
public:
property double TrueCurrentPhaseC {
	double get ();
}
```
```fsharp
member TrueCurrentPhaseC : float with get
```
