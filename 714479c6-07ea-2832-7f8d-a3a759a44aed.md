...
```
```vb
Public Interface IStructuralSectionsServer
...
```
```fsharp
type IStructuralSectionsServer = 
    interface
...
The IStructuralSectionsServer type exposes the following members.
...
|  | [StructuralSectionsUpdate](fabbe13b-5e48-1b86-6cd4-07b0a505a953.md) | The server's method that will be called when Revit User clicks the Section Type parameter's button in the family dialog. |
