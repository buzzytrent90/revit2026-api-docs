

Revit 2026 API

# ViewCropBoxActive Property  
  
---  
  
Whether or not the Crop Box/Region is active for the view.

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public bool CropBoxActive { get; set; }
```
```vb
Public Property CropBoxActive As Boolean
	Get
	Set
```
```cpp
public:
property bool CropBoxActive {
	bool get ();
	void set (bool value);
}
```
```fsharp
member CropBoxActive : bool with get, set
```


#### Property Value

Boolean

| Exception | Condition |
| --- | --- |
| [InapplicableDataException](dc1a6d15-8923-a1fe-722a-4e976634a519.md) | Thrown when getting this property, if the view is a template and cannot have a crop box applied. |
| [InvalidOperationException](9e715f03-3884-e539-4dd6-8d7545733adc.md) | Thrown when setting this property, if the view is a template and cannot have a crop box applied. |
  
Identifies whether the crop region modifies the graphical appearance of elements in the view.

#### Reference

[View Class](fb92a4e7-f3a7-ef14-e631-342179b18de9.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)