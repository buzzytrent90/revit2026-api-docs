...
# RebarSpliceTypeUtilsGetLapLengthMultiplier Method  
...
```
```vb
Public Shared Function GetLapLengthMultiplier ( 
...
```cpp
public:
static double GetLapLengthMultiplier(
	Document^ document, 
	ElementId^ rebarSpliceTypeId
)
```
```fsharp
static member GetLapLengthMultiplier : 
        document : Document * 
...
Double Returns the lap length multiplier value. 
