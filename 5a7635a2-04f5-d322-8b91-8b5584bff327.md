

Revit 2026 API

# DirectShapeAddReferencePlane(Plane, BoundingBoxUV) Method  
  
---  
  
Adds a reference plane to the DirectShape. The reference plane can either be bounded or unbounded. 

**Namespace:** [Autodesk.Revit.DB](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public void AddReferencePlane(
	Plane refPlane,
	BoundingBoxUV boundingBoxUV
)
```
```vb
Public Sub AddReferencePlane ( 
	refPlane As Plane,
	boundingBoxUV As BoundingBoxUV
)
```
```cpp
public:
void AddReferencePlane(
	Plane^ refPlane, 
	BoundingBoxUV^ boundingBoxUV
)
```
```fsharp
member AddReferencePlane : 
        refPlane : Plane * 
        boundingBoxUV : BoundingBoxUV -> unit 
```


#### Parameters

refPlane [Plane](6a6ee978-f114-558d-3c69-00d289aa855f.md)
     The geometry of the new reference plane. 
boundingBoxUV [BoundingBoxUV](e38a0145-4267-0b3f-0718-adb14e34c94e.md)
     If boundingBoxUV is set, the resulting reference plane that is added to the DirectShape will be displayed with those bounds. Note that the specified bounds must not be degenerate. If boundingBoxUV is not set, reasonable bounds are automatically calculated and applied to the input plane. The automatic bounds are based on the host direct shape's geometry. 

| Exception | Condition |
| --- | --- |
| [ArgumentException](2e6e4206-97a8-dd4b-df5d-4269f4bb6088.md) | boundingBoxUV cannot be used as a BoundingBoxUV for the reference plane surface. |
| [ArgumentNullException](631e1424-60f4-929b-4e52-dda9dcd26316.md) | A non-optional argument was null |
  
#### Reference

[DirectShape Class](bfbd137b-c2c2-71bb-6f4a-992d0dcf6ea8.md)

[AddReferencePlane Overload](f650779d-7f62-082d-28e5-d4e73e1b20ef.md)

[Autodesk.Revit.DB Namespace](87546ba7-461b-c646-cbb1-2cb8f5bff8b2.md)