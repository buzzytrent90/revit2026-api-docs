

Revit 2026 API

# DistributionSysTypeSetReverseIterator Method  
  
---  
  
Retrieve a backward moving iterator to the set.

**Namespace:** [Autodesk.Revit.DB.Electrical](212a1314-7843-2c6c-3322-363127e4059f.md)**Assembly:** RevitAPI (in RevitAPI.dll) Version: 26.0.4.0 (26.0.4.0)

```csharp
public virtual DistributionSysTypeSetIterator ReverseIterator()
```
```vb
Public Overridable Function ReverseIterator As DistributionSysTypeSetIterator
```
```cpp
public:
virtual DistributionSysTypeSetIterator^ ReverseIterator()
```
```fsharp
abstract ReverseIterator : unit -> DistributionSysTypeSetIterator 
override ReverseIterator : unit -> DistributionSysTypeSetIterator 
```


#### Return Value

[DistributionSysTypeSetIterator](40c37b27-1807-9a2b-33a6-ab77a30bcbac.md)Returns a backward moving iterator to the set.

#### Reference

[DistributionSysTypeSet Class](4e94a9fd-aaec-b38b-5d67-edac1ed07d50.md)

[Autodesk.Revit.DB.Electrical Namespace](212a1314-7843-2c6c-3322-363127e4059f.md)